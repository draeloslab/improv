...
class BayesOptimizer(Actor):
    def __init__(self, *args, stimuli=None, param_file=None, calibration=False, **kwargs):
        ...
        if self.calibration:
            logger.info('calibration stim set is: {}'.format(self.stim_space['calibration_stim']))
    ...
    def runStep(self):
        ...
        try:
            ...
            frame_num = self.client.get(ids[2])  # get the frame number from analysis
        ...
        if self.stop_sending:
            ...
            if len(nonopt) >= 1 or len(self.goback_neurons)>=1:
                ...
                logger.info('UPDATE - optim.max_acq: {}'.format(ind))
...
class RandomSampler(Actor):
    ...
    def stop(self):
        ...
        try:
            ...
            logger.error("Trouble saving optimizer timings: {}".format(e))
            pass
        logger.info('RandomSampler complete, avg time per frame: {}'.format(np.mean(self.total_times)))
...
class RandomBayesOptimizer(Actor):
    ...
    def _obs_count(self, n_idx: int) -> int:
        return int(np.count_nonzero(~np.isnan(self.y0[n_idx, :])))# TODO: make a BayesOpt actor (take optimizer class from Stimulus actor)
import time
import numpy as np
import random
import zmq
