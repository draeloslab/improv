...
class VisualStimulus(Actor):
    ...
    def send_frame(self, stim):
        if stim is not None:
            text = {'texture_size':(1024,1024), 'texture_name':'grating_gray'}
            ...
            self.timer = time.time()
            logger.info('Number of stimuli displayed: {}'.format(self.displayed_stim_num))
